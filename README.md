...
Moreover, **`-report`** works the same for PatricBrowserTools_Single_CPU; however, it does not have **`-l`** since it is a single CPU and the process can be tracked directly on the terminal.
